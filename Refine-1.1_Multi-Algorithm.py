...
def main():
    ...
    print_fire_extinguish_plan_3d(left_fire_plan, right_fire_plan, fire_time, building_3d)
    ...
    # 显示算法比较结果
    print("\n=== 算法比较结果 ===")
    print("算法名称 | 计算时间(秒) | 救援时间(秒) | 解决方案质量")
    print("-" * 60)
    for name, result in results.items():
        if result:
            print(f"{name:8} | {result['computation_time']:11.4f} | {result['rescue_time']:12.2f} | {result['solution_quality']:14}")
