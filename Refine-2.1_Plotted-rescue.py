...
class ImprovedIntelligentPeopleMovementModel:
                ...
                for i in range(self.rows):
                    for j in range(self.cols):
                        if self.building_3d[floor][i][j] == 4:
                            # 楼梯到1楼出口的距离估算
                            stair_to_exit_distance = floor * 6  # 每层楼额外距离
    ...
    def update_people_movement(self, elapsed_time, smoke_grid=None):
        ...
        current_move_prob = self.move_probability * (1 + time_factor * 0.8)  # 增加移动概率增长
    ...
    def _move_person_intelligently(self, floor, i, j, smoke_grid=None):
        ...
        if self._should_use_stairs(floor, i, j):
            stair_move = self._move_to_stairs_orderly(floor, i, j, smoke_grid)
            if stair_move:
                return stair_move
        
        # 如果是走廊人员，使用增强的走廊移动逻辑
        if self._is_in_corridor(floor, i, j):
            corridor_move = self._enhanced_corridor_move(floor, i, j, smoke_grid)
            if corridor_move:
                return corridor_move
        
        # 原有移动逻辑，但增强楼梯使用
        return self._enhanced_room_person_move(floor, i, j, smoke_grid)
    ...
    def _enhanced_room_person_move(self, floor, i, j, smoke_grid):
        ...
        for neighbor in neighbors:
            ...
            if self.building_3d[nf][ni][nj] in [0, 1, 2, 3, 4, -1]:
                ...
                distance_score = (current_dist - new_dist) * self.survival_desire * floor_urgency
                ...
                exit_bonus = 0
                if self.building_3d[nf][ni][nj] in [2, 3]:
                    exit_bonus = self.exit_attraction * floor_urgency
                elif self.building_3d[nf][ni][nj] == 4:
                    exit_bonus = self.stair_usage_bonus * floor_urgency
                    # 如果是下楼，额外奖励 - 高层人员更鼓励下楼
                    if nf < floor:
                        exit_bonus += self.floor_descent_bonus * (floor - nf) * floor_urgency
                    # 新增：如果是1楼楼梯，额外逃生奖励
                    if nf == 0:
                ...
                if self.floor_awareness and nf != floor:
                    if nf < floor:  # 下楼
                        floor_bonus = (floor - nf) * 25 * floor_urgency  # 大幅增加下楼奖励
                    else:  # 上楼 - 通常不鼓励，除非必要
                        floor_bonus = -15
                ...
                if self.building_3d[floor][i][j] == -1:  # 如果当前位置有火
                    survival_bonus = 30 * floor_urgency  # 强烈求生欲望
                ...
                total_score = (distance_score * self.intelligence_level - 
                             ...
                             survival_bonus + high_floor_bonus)
    ...
    def _enhanced_corridor_move(self, floor, i, j, smoke_grid):
        """增强的走廊人员移动逻辑"""
        # 如果在1楼走廊，直接寻找出口或楼梯
        if floor == 0:
            # 先检查当前位置是否是出口或楼梯
            ...
            return self._move_to_exit_from_corridor(floor, i, j, smoke_grid)
        else:
            # 在楼上走廊，优先下楼
            # 先检查当前位置是否是楼梯
            if self.building_3d[floor][i][j] == 4:
    ...
    def _move_to_stairs_orderly(self, floor, i, j, smoke_grid):
        """有秩序地移动到楼梯"""
        neighbors = self.path_finder._get_neighbors((floor, i, j))
    ...
    def _move_to_exit_from_corridor(self, floor, i, j, smoke_grid):
        """从走廊移动到出口"""
        # 寻找最近出口
...
class BuildingGenerator:
    ...
    def _generate_room_content(self, building, start_row, end_row, start_col, end_col, floor):
        ...
        # 调整火生成概率为10%，人员生成概率为15%
        fire_count = max(1, int(rows * cols * 0.10))  # 降低火生成概率
...
class BuildingVisualizer:
    ...
    def update_display(self, current_time, rescue_operations=None, people_states=None):
        ...
        for floor in range(self.floors):
            ...
            # 显示网格
            im = ax.imshow(display_grid, cmap=self.cmap, vmin=0, vmax=7)
    ...
    def _check_people_death(self, current_time):
        ...
        for floor, i, j, state in people_to_remove:
            if self.building_3d[floor][i][j] == 1:
                ...
                print(f"Person died: Position({floor},{i},{j}) - {death_reason} {exposure_count}")
        ...
        for i in range(room_layout["start_row"], room_layout["end_row"] + 1):
            for j in range(room_layout["start_col"], room_layout["end_col"] + 1):
                if self.building_3d[floor][i][j] == 1:
                    ...
                    print(f"Rescue executed: at time {rescue_time:.2f}s rescued person ({floor}, {i}, {j}) - room {room_key}")
...
class RealTimeRescueSimulator:
    def __init__(self, building_3d, rooms_3d, doors_3d, initial_people_count):
        ...
        self.rescue_planner = ImprovedRescuePlanner(
            copy.deepcopy(building_3d), rooms_3d, doors_3d, initial_people_count
        )
...
def print_rescue_score(rescue_score):
    ...
    print(f"Rescue Rate: {rescue_score['rescue_rate']:.2%}")
    print(f"Total Rescue Time: {rescue_score['total_rescue_time']:.2f} seconds")
    print(f"People Movement Count: {rescue_score['people_movement_count']}")
    print(f"Rescue Operations: {rescue_score['rescue_operations']}")
    print(f"Successful Rescues: {rescue_score['successful_rescues']}")
    print(f"Failed Rescues: {rescue_score['failed_rescues']}")
...
def main():
    ...
    simulator = RealTimeRescueSimulator(
        building_3d, rooms_3d, doors_3d, generator.total_people
    )
