from building_importer import BuildingDataImporter
...
class RescueRouteVisualizer:
    ...
    def visualize_floor_with_routes(self, floor_grid, routes=None, title="楼层救援路线", floor_num=0):
        """可视化单层楼的救援路线 - 修复版本"""
        rows = len(floor_grid)
        cols = len(floor_grid[0])
        
        # 创建图形
        fig, ax = plt.subplots(figsize=(max(10, cols), max(8, rows)))
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                # 添加文本标签
                if cell_value != 0:
                    label = self.labels.get(cell_value, '')
                    ax.text(j + 0.5, rows-1-i + 0.5, label,
                           ha='center', va='center', fontsize=24,
                           color='white' if cell_value in [1] else 'black')
        
        # 绘制救援路线
        if routes:
            print(f"Visualizing {len(routes)} routes for floor {floor_num}")
            for route_idx, route in enumerate(routes):
                if not route:
                    print(f"Route {route_idx} is empty")
                    continue
                    
                print(f"Route {route_idx} has {len(route)} points")
                
                path_coords = []
                for pos in route:
                    if len(pos) == 3:  # (floor, i, j)
                        pos_floor, i, j = pos
                        # 只绘制当前楼层的路径点
                        if pos_floor == floor_num:
                            path_coords.append((j + 0.5, rows-1-i + 0.5))
                    else:  # (i, j)
                        i, j = pos
                        path_coords.append((j + 0.5, rows-1-i + 0.5))
                
                print(f"Route {route_idx} has {len(path_coords)} points on floor {floor_num}")
                
                # 绘制路径线条
                if len(path_coords) > 1:
                    color = plt.cm.Set1(route_idx % 2  +1)  # 使用不同颜色区分多条路线
                    for k in range(len(path_coords) - 1):
                        ax.plot([path_coords[k][0], path_coords[k+1][0]], 
                               [path_coords[k][1], path_coords[k+1][1]], 
                               color=color, linewidth=10, alpha=0.7, 
                               label=f'救援路线 {route_idx+1}' if k == 0 else "")
                    
                    # 绘制路径点
                    for coord_idx, coord in enumerate(path_coords):
                        marker_size = 10 if coord_idx == 0 or coord_idx == len(path_coords)-1 else 6
                        marker_style = 's' if coord_idx == 0 else 'o' if coord_idx == len(path_coords)-1 else 'o'
                        ax.plot(coord[0], coord[1], marker_style, 
                               color=color, markersize=marker_size, alpha=0.7)
                        
                elif len(path_coords) == 1:
                    # 如果只有一个点，只绘制起点
                    color = plt.cm.Set1(route_idx % 9)
                    ax.plot(path_coords[0][0], path_coords[0][1], 's', 
        ...
        ax.set_ylim(0, rows)
        ax.set_aspect('equal')
        ax.invert_yaxis()  # 反转y轴使(0,0)在左上角
        
        # 添加标题和标签
        ax.set_title(f'{title} - 楼层 {floor_num+1}', fontsize=16, fontweight='bold')
        ...
        # 添加统计信息
        fire_count = sum(row.count(-1) for row in floor_grid)
        people_count = sum(row.count(1) for row in floor_grid)
        empty_count = sum(row.count(0) for row in floor_grid)
        stair_count = sum(row.count(4) for row in floor_grid)
        obstacle_count = sum(row.count(-3) for row in floor_grid)
        ...
        # 添加图例
        legend_elements = [
            patches.Patch(facecolor='blue', edgecolor='black', label='人员'),
            patches.Patch(facecolor='red', edgecolor='black', label='火源'),
            patches.Patch(facecolor='green', edgecolor='black', label='出口'),
            patches.Patch(facecolor='orange', edgecolor='black', label='楼梯'),
            patches.Patch(facecolor='gray', edgecolor='black', label='障碍物'),
            patches.Patch(facecolor='white', edgecolor='black', label='空地')
        ]
                ...
                if route:
                    ...
                    legend_elements.append(
                        plt.Line2D([0], [0], color=color, linewidth=3, 
                                  label=f'救援路线 {route_idx+1}')
                    )
            
            legend_elements.extend([
                plt.Line2D([0], [0], color='green', marker='s', linestyle='None', 
    ...
    def visualize_rescue_routes(self, filename_prefix="rescue_routes"):
        """可视化所有楼层的救援路线 - 修复版本"""
        for floor in range(self.floors):
            if floor not in self.rescue_routes:
                continue
                
            floor_routes = []
            floor_name = f"楼层 {floor+1}"
            
            # 获取左侧和右侧救援路线
            left_route = self.rescue_routes[floor]['left']
            right_route = self.rescue_routes[floor]['right']
            
            if left_route:
                print(f"Floor {floor} left route has {len(left_route)} points")
                floor_routes.append(left_route)
            if right_route:
                print(f"Floor {floor} right route has {len(right_route)} points")
                floor_routes.append(right_route)
            
            if floor_routes:
                # 创建当前楼层的网格副本用于可视化
                floor_grid = copy.deepcopy(self.building_3d[floor])
                
                print(f"Visualizing floor {floor} with {len(floor_routes)} routes")
                
                # 可视化当前楼层
                fig, ax = self.visualizer.visualize_floor_with_routes(
                    floor_grid, 
                    routes=floor_routes,
                    title="救援路线图",
                    floor_num=floor
                ...
                # 保存图像
                plt.savefig(f"{filename_prefix}_floor_{floor+1}.png", dpi=300, bbox_inches='tight')
                print(f"Saved rescue routes visualization for floor {floor+1}")
                ...
                print(f"No routes to visualize for floor {floor}")
...
class SprinklerSystem:
    def __init__(self, building_3d, rooms_3d):
        ...
        self.total_fires_extinguished = {}  # 记录每个洒水器灭火总数 {位置: 数量}
    ...
    def update(self, smoke_grid, elapsed_time):
        """更新洒水器状态，返回本轮灭火数量"""
        total_extinguished_this_round = 0
        ...
        for sprinkler_pos in available_sprinklers:
            ...
            if (floor < len(smoke_grid) and i < len(smoke_grid[floor]) and
                ...
                self.total_fires_extinguished[sprinkler_pos] = 0  # 初始化计数器

        # 更新激活的洒水器
        sprinklers_to_remove = []
        for sprinkler_pos, remaining_time in self.active_sprinklers.items():
            # 使用实际经过的时间递减
            self.active_sprinklers[sprinkler_pos] -= elapsed_time

            # 洒水器工作期间灭火
            fires_extinguished = self._extinguish_fire_around(sprinkler_pos)
            total_extinguished_this_round += fires_extinguished

            # 累计灭火数量
            if sprinkler_pos in self.total_fires_extinguished:
                self.total_fires_extinguished[sprinkler_pos] += fires_extinguished

            if self.active_sprinklers[sprinkler_pos] <= 0:
                sprinklers_to_remove.append(sprinkler_pos)
                total_count = self.total_fires_extinguished.get(sprinkler_pos, 0)
                print(f"Sprinkler at {sprinkler_pos} finished working, extinguished {total_count} fires in total")

        for sprinkler_pos in sprinklers_to_remove:
            del self.active_sprinklers[sprinkler_pos]

        return total_extinguished_this_round
...
class ImprovedIntelligentPeopleMovementModel:
    def __init__(self, building_3d, rooms_3d, exits):
        self.building_3d = building_3d
        self.original_building = copy.deepcopy(building_3d)  # 保存原始建筑数据
        self.rooms_3d = rooms_3d
        self.exits = exits
        self.floors = len(building_3d)
        self.rows = len(building_3d[0])
    ...
    def _move_person_intelligently(self, floor, i, j, smoke_grid=None, person_type='adult'):
        ...
        # 检查当前位置是否在楼梯上（通过检查原始建筑数据）
        # 因为人员移动后会覆盖楼梯标记，所以需要检查原始数据
        pos_key = (floor, i, j)
        is_on_stair = False
        ...
        # 楼梯位置在走廊中间行
        corridor_mid_row = 13
        if i == corridor_mid_row and (j == 1 or j == self.cols - 2):
            is_on_stair = True

        # 如果在楼梯上且在一楼，直接逃生
        if is_on_stair and floor == 0:
    ...
    def _perform_stair_descent(self, floor, i, j):
        ...
        if self.building_3d[floor][i][j] == 1:
            ...
            if next_floor >= 0:  # 确保不会下到负楼层
                # 检查下一层楼梯位置是否可用（空地、楼梯或已有人）
                # 允许多人同时在楼梯上，通过在楼梯周围寻找空位
                target_positions = [(next_floor, i, j)]

                # 如果楼梯位置被占用，尝试楼梯周围的位置
                if self.building_3d[next_floor][i][j] not in [0, 4]:
                    for di, dj in [(0, 1), (0, -1), (1, 0), (-1, 0)]:
                        ni, nj = i + di, j + dj
                        if (0 <= ni < self.rows and 0 <= nj < self.cols and
                            self.building_3d[next_floor][ni][nj] in [0, 4]):
                            target_positions.append((next_floor, ni, nj))

                # 尝试移动到任一可用位置
                for target_floor, target_i, target_j in target_positions:
                    if self.building_3d[target_floor][target_i][target_j] in [0, 4]:
                        # 从当前位置移除人员，恢复原有标记
                        # 检查原始楼层数据以确定恢复什么值
                        if (floor < len(self.original_building) and
                            i < len(self.original_building[floor]) and
                            j < len(self.original_building[floor][i]) and
                            self.original_building[floor][i][j] == 4):
                            self.building_3d[floor][i][j] = 4  # 恢复楼梯标记
                        else:
                            self.building_3d[floor][i][j] = 0  # 恢复空地

                        # 在下一层目标位置放置人员
                        self.building_3d[target_floor][target_i][target_j] = 1

                        # 更新人员状态和类型
                        old_pos = (floor, i, j)
                        new_pos = (target_floor, target_i, target_j)
                        ...
                        print(f"Person descended from floor {floor} to floor {next_floor} at position ({target_i},{target_j})")
    ...
    def _move_to_stairs_orderly(self, floor, i, j, smoke_grid, person_type='adult'):
        ...
        for neighbor in neighbors:
            ...
            # 如果是楼梯（检查原始位置是否是楼梯）
            corridor_mid_row = 13
            if ni == corridor_mid_row and (nj == 1 or nj == self.cols - 2):
        ...
        if stair_moves:
            ...
            if best_stair is None:
                # 所有相邻楼梯都被严重阻挡，尝试寻找其他楼梯
                print(f"All adjacent stairs blocked at ({floor},{i},{j}), searching for alternative stairs")
                all_stairs = self._find_nearest_stairs(floor, i, j)
                if all_stairs:
                    # 尝试移动到最近的未被阻挡的楼梯
                    for stair_pos in all_stairs:
                        sf, si, sj = stair_pos
                        if not self._is_stair_blocked_by_fire(sf, si, sj):
                            # 使用A*寻路到这个楼梯
                            path, _ = self.path_finder.find_path(
                                (floor, i, j), stair_pos,
                                has_people=False, smoke_grid=None
                            )
                            if path and len(path) > 1:
                                next_pos = path[1]
                                nf, ni, nj = next_pos
                                if self.building_3d[nf][ni][nj] in [0, 4]:
            ...
            # 直接移动并下楼（一步完成）
            if sf == floor and floor > 0:  # 同层楼梯且不在1楼
                # 直接下楼到下一层
                next_floor = floor - 1
                if self.building_3d[next_floor][si][sj] in [0, 4]:
                    # 保存人员类型
                    person_type_saved = self.get_person_type((floor, i, j))

                    # 移除当前位置的人员
                    self.building_3d[floor][i][j] = 0
                    # 在下一层楼梯位置放置人员
                    self.building_3d[next_floor][si][sj] = 1

                    # 更新人员状态和类型
                    old_pos = (floor, i, j)
                    new_pos = (next_floor, si, sj)
                    if old_pos in self.people_states:
                        self.people_states[new_pos] = self.people_states[old_pos]
                        del self.people_states[old_pos]
                    if old_pos in self.people_types:
                    ...
                    print(f"Person moved to stairs and descended from floor {floor} to floor {next_floor} at position ({si},{sj})")
                    return "stair_descent"
            elif sf == floor and floor == 0:  # 同层楼梯且在1楼
                # 直接逃生
                self.building_3d[floor][i][j] = 0
                if (floor, i, j) in self.people_states:
                    del self.people_states[(floor, i, j)]
                if (floor, i, j) in self.people_types:
                    del self.people_types[(floor, i, j)]
                print(f"Person moved to stairs and escaped at floor {floor}, position ({si},{sj})")
                return "stair_escape"

        return False

    def _is_stair_blocked_by_fire(self, floor, i, j):
        """检查楼梯是否被火阻挡"""
        # 检查楼梯位置本身
        if self.building_3d[floor][i][j] == -1:
            print(f"Stair at ({floor},{i},{j}) is ON FIRE")
            return True
        ...
        # 如果周围有超过3个火焰格子，认为被严重阻挡
        if fire_count > 8:
            print(f"Stair at ({floor},{i},{j}) is HEAVILY BLOCKED by {fire_count} fire cells")
            return True

        # 检查下一层楼梯是否被火阻挡（如果要下楼）
        if floor > 0:
            next_floor = floor - 1
            if self.building_3d[next_floor][i][j] == -1:
                print(f"Stair at ({floor},{i},{j}) has FIRE BELOW at floor {next_floor}")
                return True
            ...
            for di in range(-1, 2):
                for dj in range(-1, 2):
                    ni, nj = i + di, j + dj
                    if (0 <= ni < self.rows and 0 <= nj < self.cols and
                        self.building_3d[next_floor][ni][nj] == -1):
                        fire_count_below += 1

            if fire_count_below > 6:
                print(f"Stair at ({floor},{i},{j}) has FIRE BELOW (floor {next_floor}): {fire_count_below} fire cells")
                return True
    ...
    def _select_best_stair_orderly(self, floor, i, j, stair_positions):
        ...
        for stair_pos in stair_positions:
            ...
            # 检查楼梯是否被火阻挡
            is_blocked = self._is_stair_blocked_by_fire(sf, si, sj)
            ...
            if is_blocked:
                # 被阻挡的楼梯得分很低，但不完全排除
                total_score = -60 + distance_score * 0.1
            else:
                total_score = distance_score - crowding_penalty
                unblocked_stairs.append(stair_pos)

            stair_scores.append((stair_pos, total_score))

        # 如果所有楼梯都被阻挡，返回None
        if not unblocked_stairs and all(score < -50 for _, score in stair_scores):
            return None

        # 选择得分最高的楼梯
        stair_scores.sort(key=lambda x: x[1], reverse=True)
        return stair_scores[0][0]
    ...
    def _find_nearest_stairs(self, floor, i, j):
        """找到最近的楼梯位置 - 优先选择未被火阻挡的楼梯"""
        stair_positions = []

        # 楼梯通常在走廊中间行的两端
        corridor_mid_row = 13
        potential_stairs = [
            (floor, corridor_mid_row, 1),           # 左侧楼梯
            (floor, corridor_mid_row, self.cols - 2) # 右侧楼梯
        ]
        ...
        for stair_i in range(self.rows):
            for stair_j in range(self.cols):
                if self.building_3d[floor][stair_i][stair_j] == 4:
                    pos = (floor, stair_i, stair_j)
                    if pos not in potential_stairs:
                        potential_stairs.append(pos)

        # 按照是否被火阻挡和距离排序
        stair_scores = []
        for stair_pos in potential_stairs:
            sf, si, sj = stair_pos
            ...
            # 计算距离
            distance = abs(si - i) + abs(sj - j)

            # 未被阻挡的楼梯优先级高
            if is_blocked:
                score = distance + 1000  # 被阻挡的楼梯得分很低
            else:
                score = distance

            stair_scores.append((stair_pos, score))

        # 按得分排序
        stair_scores.sort(key=lambda x: x[1])

        # 返回排序后的楼梯位置
        return [pos for pos, score in stair_scores]
            ...
            for i in range(len(self.building_3d[floor])):
                for j in range(len(self.building_3d[floor][0])):
                    if self.building_3d[floor][i][j] == 1:
                        ...
                        if (state['in_fire_count'] >= 30 or 
                            state['consecutive_fire_exposure'] >= 15):
...
class FireSpreadModel:
    def __init__(self, building_3d):
        ...
        self.rows = len(building_3d[0])
        self.cols = len(building_3d[0][0])
        self.k = 0.01  # 火焰蔓延速度（每秒增长1%）
        self.fire_areas = []
        
    def update_fire_spread(self, time_step):
        # 更新火源列表
        self.fire_sources = self._identify_fire_sources()
        self._identify_fire_areas()

        for fire_area in self.fire_areas:
            self._spread_fire_area(fire_area, time_step)
    ...
    def _spread_fire_area(self, fire_area, time_step):
        if not fire_area:
            return
        ...
        # 计算本次应该蔓延的格子数
        # 使用固定的k=0.01，每秒蔓延当前火焰面积的1%
        current_area = len(fire_area)

        # 每秒蔓延的格子数 = 当前面积 * k
        # time_step 是时间增量（秒）
        cells_per_second = max(1, current_area * self.k)
        new_fire_cells = int(cells_per_second * time_step)

        # 确保至少蔓延1格（如果有候选位置且时间步长足够）
        if time_step >= 1.0:
            new_fire_cells = max(1, new_fire_cells)
        ...
        if total_weight > 0:
            ...
            if spread_count > 0:
                print(f"Fire spread: {spread_count} new cells ignited (total fire area: {current_area + spread_count})")
...
class BuildingGenerator:
    def __init__(self):
        self.floors = random.randint(3, 12)
        # self.floors = 12
        self.rooms_per_side = random.randint(2, 5)
        # self.rooms_per_side = 5
        self.rows = 27
    ...
    def _generate_room_content(self, building, start_row, end_row, start_col, end_col, floor):
        ...
        # 调整人员生成概率为15%
        people_count = max(1, int(rows * cols * 0.02))  # 增加人员生成概率
    ...
    def _generate_fire_seeds(self, building, floor):
        ...
        target_fire_cells = int(total_cells * random.uniform(0,0.03))  # 目标火焰面积20%
...
class BuildingVisualizer:
    ...
    def save_initial_state(self, filename="building_initial_state.png"):
        """保存0秒的建筑初始状态"""
        # 创建新的图形用于保存
        if self.floors <= 3:
            fig, axs = plt.subplots(1, self.floors, figsize=(5*self.floors, 5))
        else:
            rows = (self.floors + 1) // 2
        ...
        for floor in range(self.floors):
            ...
            ax.set_title(f'Floor {floor+1} - Initial State (0s)')
        ...
        # 隐藏多余的子图
        for i in range(self.floors, len(axs)):
        ...
        print(f"Initial building state saved as {filename}")
        
        # 关闭图形以释放内存
        plt.close(fig)
        
    def update_display(self, current_time, rescue_operations=None, people_states=None, sprinkler_status=None, building_3d=None):
        ...
        # 更新building_3d引用，使用最新的数据
        if building_3d is not None:
            self.building_3d = building_3d
        ...
        for floor in range(self.floors):
            ...
            for (f, i, j), state in self.people_states.items():
                if f == floor:
                    ...
                    # 只有当exposure或consecutive不为0时才显示
                    if exposure > 0 or consecutive > 0:
...
class ImprovedRescuePlanner:
    def __init__(self, building_3d, rooms_3d, doors_3d, initial_people_count, people_type_mapping=None):
        ...
        # 修改救援人员起始位置 - 为每个楼层创建一对救援人员
        self.rescuer_pairs = {}  # 存储每个楼层的救援人员对
        
        for floor in range(self.floors):
            # 左侧救援人员
            left_rescuer_start = (floor, corridor_mid_row, 0)
            # 右侧救援人员
            if len(building_3d) > 0 and len(building_3d[floor]) > 0:
                right_start_col = len(building_3d[floor][0]) - 1
                right_rescuer_start = (floor, corridor_mid_row, right_start_col)
            else:
                right_rescuer_start = (floor, corridor_mid_row, 35)
            
            self.rescuer_pairs[floor] = {
                'left': left_rescuer_start,
                'right': right_rescuer_start
            }
        ...
        self.rescue_routes = {}  # 记录每个楼层的救援路线
        self.visualizer = RescueRouteVisualizer()
    ...
    def get_room_info(self):
        ...
        for floor in range(len(self.rooms_3d)):
            for room_id, room_data in self.rooms_3d[floor].items():
                ...
                # 增强高层房间的重要性计算
                floor_importance = (self.floors - floor) * 20  # 楼层越高，重要性越大
                ...
                room_info[room_key] = {
                    ...
                    "importance": people_count * 100 + floor_importance + fire_count * 2,
                    # 添加边界信息
                    "start_row": start_row,
                    "end_row": end_row,
                    "start_col": start_col,
    ...
    def should_extinguish_fire_on_path(self, path, current_time):
        """评估是否应该在路径上灭火"""
        fire_cells_on_path = []
        ...
        for pos in path:
            floor, i, j = pos
            if self.building_3d[floor][i][j] == -1:
                fire_cells_on_path.append(pos)

        if not fire_cells_on_path:
            return [], 0
        ...
        for fire_pos in fire_cells_on_path:
            floor, fi, fj = fire_pos
            # 检查火焰周围5格范围内的人数
            for di in range(-5, 6):
                for dj in range(-5, 6):
                    ni, nj = fi + di, fj + dj
                    if (0 <= ni < self.rows and 0 <= nj < self.cols and
                        ...
                        if distance <= 2:
                            people_blocked += 1  # 非常近的人被阻挡
                        people_at_risk += 1

        # 计算灭火时间
        extinguish_time = len(fire_cells_on_path) * 5  # 每格5秒

        # 决策逻辑：
        # 1. 如果有人被直接阻挡（2格内），优先灭火（即使时间较长）
        # 2. 如果周围有较多人（>3人）且灭火时间合理（不超过50秒），则灭火
        # 3. 如果火焰数量少（<=3格）且周围有人，则灭火

        if people_blocked > 0 and extinguish_time <= 60:
            print(f"Fire blocking decision: {people_blocked} people blocked, will extinguish {len(fire_cells_on_path)} fires")
            return fire_cells_on_path, extinguish_time

        if people_at_risk >= 3 and extinguish_time <= 50:
            print(f"Fire risk decision: {people_at_risk} people at risk, will extinguish {len(fire_cells_on_path)} fires")
            return fire_cells_on_path, extinguish_time

        if len(fire_cells_on_path) <= 3 and people_at_risk > 0:
            print(f"Small fire decision: {len(fire_cells_on_path)} fires, {people_at_risk} people at risk, will extinguish")
            return fire_cells_on_path, extinguish_time

        return [], 0

    def extinguish_fires(self, fire_positions, current_time):
        """灭火操作"""
        ...
        for pos in fire_positions:
            ...
            if self.building_3d[floor][i][j] == -1:
                ...
                print(f"Fire extinguished at ({floor},{i},{j}) at time {current_time:.2f}s")
    ...
    def execute_rescue_operation(self, room_key, room_info, rescue_time):
        ...
        # 检查房间门口是否有火，如果有则先灭火
        door_pos = room_data['door_position']
        door_floor, door_i, door_j = door_pos
        fires_to_extinguish = []

        # 检查门口周围1格范围的火焰
        for di in [-1, 0, 1]:
            for dj in [-1, 0, 1]:
                ni, nj = door_i + di, door_j + dj
                if (0 <= ni < self.rows and 0 <= nj < self.cols and
                    self.building_3d[door_floor][ni][nj] == -1):
                    fires_to_extinguish.append((door_floor, ni, nj))

        # 如果门口有火，先灭火
        if fires_to_extinguish:
            extinguish_count = self.extinguish_fires(fires_to_extinguish, rescue_time)
            extinguish_time = extinguish_count * 5
            rescue_time += extinguish_time
            print(f"Extinguished {extinguish_count} fires near room {room_key} door, time cost: {extinguish_time:.2f}s")
    ...
    def execute_complete_rescue_plan(self, floor_plans, room_info):
        """执行完整的救援计划 - 每个楼层由专门的救援人员对负责"""
        print(f"Starting complete rescue plan: {len(floor_plans)} floors with dedicated rescuer pairs")
        
        # 重置模拟时间
        self.current_simulation_time = 0
        self.last_update_time = 0
        
        self.rescue_routes = {floor: {'left': [], 'right': []} for floor in range(self.floors)}
        # 更新人员跟踪
        ...
        for floor in range(self.floors):
            ...
            print(f"\n=== Starting rescue operations for Floor {floor} ===")
            
            # 获取该楼层的救援计划
            floor_plan = floor_plans[floor]
            left_rooms = floor_plan.get('left_rooms', [])
            right_rooms = floor_plan.get('right_rooms', [])
            
            # 获取该楼层的救援人员起始位置
            rescuer_starts = self.rescuer_pairs[floor]
            left_start = rescuer_starts['left']
            right_start = rescuer_starts['right']
            
            # 执行左侧救援路线并记录
            left_route = self._execute_and_record_rescue_route(
                left_rooms, room_info, left_start, f"Floor{floor}-Left", floor, 'left')
            
            # 执行右侧救援路线并记录  
            right_route = self._execute_and_record_rescue_route(
                right_rooms, room_info, right_start, f"Floor{floor}-Right", floor, 'right')
            
            # 记录路线
            self.rescue_routes[floor]['left'] = left_route
            self.rescue_routes[floor]['right'] = right_route
            ...
            left_time = self._execute_rescue_route(
                left_rooms, room_info, left_start, f"Floor{floor}-Left")
            
            # 执行右侧救援路线  
            right_time = self._execute_rescue_route(
                right_rooms, room_info, right_start, f"Floor{floor}-Right")
            
            floor_rescue_times[floor] = max(left_time, right_time)
            print(f"Floor {floor} rescue completed in {floor_rescue_times[floor]:.2f}s")
        
        # 取所有楼层救援时间的最大值作为总时间
        total_time = max(floor_rescue_times.values()) if floor_rescue_times else 0
    ...
    def _execute_and_record_rescue_route(self, room_sequence, room_info, start_pos, route_name, floor_num, route_type):
        """执行救援路线并记录路径 - 修复版本"""
        current_time = 0
        current_pos = start_pos
        route_path = [start_pos]  # 记录路径
            ...
            if current_pos[0] != floor:
                ...
                if travel_time != float('inf'):
                    ...
                    for step_pos in path:
                        if step_pos not in route_path:  # 避免重复记录
                            route_path.append(step_pos)
        ...
        print(f"{route_name} completed with {len(route_path)} path points")
        return route_path
    ...
    def _update_environment_to_time(self, target_time):
        ...
        while self.current_simulation_time < target_time:
            ...
            # 更新洒水器系统
            sprinkler_fires_extinguished = self.sprinkler_system.update(self.smoke_model.smoke_grid_3d, time_step)
            self.fires_extinguished += sprinkler_fires_extinguished
    ...
    def _simulate_after_rescue(self, rescue_end_time):
        ...
        max_consecutive_no_activity = 50  # 连续50次（100秒）无活动则停止
        ...
        for additional_time in range(0, max_additional_time, self.update_interval):
            ...
            # 更新洒水器系统
            sprinkler_fires_extinguished = self.sprinkler_system.update(self.smoke_model.smoke_grid_3d, self.update_interval)
            self.fires_extinguished += sprinkler_fires_extinguished
            ...
            # 检查是否有活动
            has_activity = (self_rescue_count > 0 or people_moved > 0)

            if has_activity:
                consecutive_no_activity = 0  # 重置计数器
            else:
                consecutive_no_activity += 1
                print(f"No activity detected ({consecutive_no_activity}/{max_consecutive_no_activity})")
            ...
            # 如果连续无活动达到阈值，停止模拟
            if consecutive_no_activity >= max_consecutive_no_activity:
                print(f"No activity for {max_consecutive_no_activity} consecutive updates, ending simulation")
                break
...
class MultiAStarRescuePlanner:
    ...
    def solve(self):
        ...
        for floor, floor_rooms in rooms_by_floor.items():
            print(f"\n=== Generating rescue plan for Floor {floor} ===")
            
            # 计算当前楼层的总人数
            floor_people_count = sum(room_info[room_key]['people_count'] for room_key in floor_rooms)
            print(f"Floor {floor}: {len(floor_rooms)} rooms, {floor_people_count} people")
            
            # 按重要性排序当前楼层的房间
            sorted_rooms = sorted(floor_rooms,
                                key=lambda x: (-room_info[x]['people_count'],  # 房间人数多的优先
                                             -room_info[x]['importance']))     # 重要性高的优先
            ...
            for i, room_key in enumerate(sorted_rooms):
                ...
                # 根据房间位置分配到左或右
                # 确保房间数据包含边界信息
                if 'start_col' in room_data and 'end_col' in room_data:
                    room_center_col = (room_data["start_col"] + room_data["end_col"]) // 2
                    building_cols = len(self.building_3d[floor][0])
                    
                    if room_center_col < building_cols // 2:
                        left_rooms.append(room_key)
                    else:
                        right_rooms.append(room_key)
                else:
                    # 如果房间没有边界信息，根据房间ID分配到左或右
                    if room_data['room_id'] <= len(floor_rooms) // 2:
                        left_rooms.append(room_key)
                    else:
                        right_rooms.append(room_key)
            ...
            # 优化序列
            corridor_mid_row = 13
            left_start = (floor, corridor_mid_row, 0)
            right_start = (floor, corridor_mid_row, len(self.building_3d[floor][0]) - 1)
            
            left_rooms = self._optimize_sequence(left_rooms, left_start, room_info, {floor: floor_people_count})
            right_rooms = self._optimize_sequence(right_rooms, right_start, room_info, {floor: floor_people_count})
            
            floor_plans[floor] = {
                'left_rooms': left_rooms,
                'right_rooms': right_rooms,
                'total_people': floor_people_count
            }
            
            print(f"Floor {floor} - Left route: {len(left_rooms)} rooms, Right route: {len(right_rooms)} rooms")

        return floor_plans, room_info
    
    def _optimize_sequence(self, rooms, start_pos, room_info, floor_people_count):
        if len(rooms) <= 1:
            return rooms
        ...
        # 按楼层总人数和楼层高度排序（人数多的楼层优先，人数相同时高层优先）
        sorted_floors = sorted(rooms_by_floor.keys(),
                              key=lambda f: (-floor_people_count.get(f, 0), -f))
        ...
        for floor in sorted_floors:
            floor_rooms = set(rooms_by_floor[floor])

            # 在同一楼层内，优先救人多的房间，其次按距离优化
            while floor_rooms:
                # 综合考虑人数和距离
                best_room = min(floor_rooms,
                               key=lambda x: (
                                   -room_info[x]['people_count'] * 10,  # 人数权重
                                   self._estimate_distance(current_pos, x, room_info)  # 距离权重
                               ))
                optimized_sequence.append(best_room)
                floor_rooms.remove(best_room)
                current_pos = room_info[best_room]['door_position']
...
class RealTimeRescueSimulator:
    def __init__(self, building_3d, rooms_3d, doors_3d, initial_people_count, people_type_mapping=None):
        ...
        self.rescue_planner = ImprovedRescuePlanner(
            copy.deepcopy(building_3d), 
            rooms_3d, 
            doors_3d, 
            initial_people_count, 
            people_type_mapping  # 这里传递人员类型映射
        )
        ...
        self.floor_plans, _ = self.multi_astar_planner.solve()
        
        self.current_time = 0
        self.update_interval = 2
        
    def run_simulation(self):
        print("Starting real-time rescue simulation with dedicated rescuer pairs per floor...")
        
        # 保存0秒的初始状态
        self.visualizer.save_initial_state("building_initial_state.png")
        
        # 打印每个楼层的救援计划
        for floor, plan in self.floor_plans.items():
            print(f"Floor {floor}: Left route - {len(plan['left_rooms'])} rooms, Right route - {len(plan['right_rooms'])} rooms, Total people: {plan['total_people']}")
        ...
        self.visualizer.update_display(
            ...
            building_3d=self.rescue_planner.building_3d
        )

        # 初始环境更新
        print("Initial environment update (fire spreading and people starting to move)...")
        initial_update_time = 5.0
        self._update_environment_to_time(initial_update_time)
        self.current_time = initial_update_time
        ...
        # 执行每个楼层的救援
        total_time = self.rescue_planner.execute_complete_rescue_plan(self.floor_plans, self.room_info)
        
        # 后续模拟
        final_time = self._simulate_after_rescue(total_time)
        
        # 显示救援路线可视化
        print("\n=== Generating Rescue Route Visualizations ===")
        self.rescue_planner.visualize_rescue_routes()
    ...
    def _execute_rescue_route(self, room_sequence, start_pos, route_name):
        ...
        for room_key in room_sequence:
            ...
            if floor not in rooms_by_floor:
                rooms_by_floor[floor] = []
                floor_order.append(floor)  # 记录楼层首次出现的顺序
            rooms_by_floor[floor].append(room_key)

        # 打印救援顺序
        print(f"{route_name} rescue order by floor:")
        for floor in floor_order:
            floor_people = sum(self.room_info[room_key]['people_count'] for room_key in rooms_by_floor[floor])
            print(f"  Floor {floor}: {len(rooms_by_floor[floor])} rooms, {floor_people} people")

        # 按照房间序列中楼层出现的顺序救援（保持优先级）
        for floor in floor_order:
            print(f"{route_name} rescuer moving to floor {floor}")
            ...
            if current_pos[0] != floor:
                ...
                if travel_time != float('inf'):
                    # 检查路径上是否需要灭火
                    fires_to_extinguish, extinguish_time = self.rescue_planner.should_extinguish_fire_on_path(
                        path, current_time
                    )

                    if fires_to_extinguish:
                        print(f"{route_name} rescuer: Will extinguish {len(fires_to_extinguish)} fires on path to floor {floor}, time cost: {extinguish_time:.2f}s")

                    # 模拟移动过程
                    step_time = current_time
                    fire_extinguished_on_path = False
                    ...
                    for step_idx, step_pos in enumerate(path[1:], 1):
                        ...
                        # 如果这个位置需要灭火且还没灭过
                        if not fire_extinguished_on_path and step_pos in fires_to_extinguish:
                            # 灭掉路径上的所有火
                            extinguished = self.rescue_planner.extinguish_fires(fires_to_extinguish, step_time)
                            if extinguished > 0:
                                step_time += extinguish_time
                                current_time += extinguish_time
                                fire_extinguished_on_path = True
                                print(f"{route_name} rescuer extinguished {extinguished} fires on path to floor {floor} at time {step_time:.2f}s")
            ...
            for room_key in rooms_by_floor[floor]:
                ...
                if fires_to_extinguish:
                    print(f"{route_name} rescue: Will extinguish {len(fires_to_extinguish)} fires on path, time cost: {extinguish_time:.2f}s")
                ...
                for step_idx, step_pos in enumerate(path[1:], 1):
                    ...
                    if not fire_extinguished_on_path and step_pos in fires_to_extinguish:
                        ...
                        if extinguished > 0:
                            ...
                            current_time += extinguish_time
                            fire_extinguished_on_path = True
                            print(f"{route_name} rescuer extinguished {extinguished} fires on path at time {step_time:.2f}s")
    ...
    def _update_environment_to_time(self, target_time):
        # 添加连续无活动计数器
        consecutive_no_activity = 0
        max_consecutive_no_activity = 50  # 连续50次（100秒）无活动则停止  # 连续10次无活动则停止
        ...
        while self.rescue_planner.current_simulation_time < target_time:
            ...
            print(f"Updating environment: time={self.rescue_planner.current_simulation_time:.2f}s, time_step={time_step:.2f}s")
            ...
            # 更新洒水器系统
            sprinkler_fires_extinguished = self.rescue_planner.sprinkler_system.update(
                self.rescue_planner.smoke_model.smoke_grid_3d, time_step)
            self.rescue_planner.fires_extinguished += sprinkler_fires_extinguished
            ...
            print(f"  People moved: {people_moved}, Self rescued: {self_rescue_count}")
            ...
            if has_activity:
                ...
                consecutive_no_activity += 1
                if consecutive_no_activity >= max_consecutive_no_activity:
                    print(f"No activity for {max_consecutive_no_activity} consecutive updates, stopping environment update")
                    break
            ...
            if self.rescue_planner.current_people_count <= 0:
                print("All people processed, stopping environment update")
                break
...
def print_rescue_score(rescue_score):
    ...
    print(f"Casualty: {rescue_score['current_people']}")
    ...
    if 'people_type_stats' in rescue_score:
        print("\n=== People Type Statistics ===")
        for person_type, stats in rescue_score['people_type_stats'].items():
            total = stats['saved'] + stats['self_rescued']
            print(f"{person_type.capitalize()}: Saved: {stats['saved']}, Self Rescued: {stats['self_rescued']}, Total: {total}")
...
def main():
    print("Fire Simulator: Real-Time Rescue Simulation")
    print("=" * 50)
    
    # 创建建筑数据导入器
    importer = BuildingDataImporter()
    
    # 选择数据源类型
    print("Please choose the way to generate or import the building:")
    print("1. random generate")
    print("2. import from Json")
    print("3. import from Image")
    
    choice = input("Enter number(1-3): ").strip()
    
    if choice == "2":
        # 从JSON文件导入
        json_file = input("Enter the path of Json").strip()
        try:
            building_3d, rooms_3d, doors_3d = importer.import_building_data(json_file, 'json')
            print("Successfully imported")
        except Exception as e:
            print(f"Failed importing: {e}")
        ...
        # 从图像导入
        image_file = input("Enter the path: ").strip()
        num_floors = int(input("Enter number of layers:") or "3")
        try:
            building_3d, rooms_3d, doors_3d,people_type_mapping = importer.import_building_data(image_file, 'image', num_floors=num_floors)
            print("Successfully imported")
        except Exception as e:
            print(f"Failed: {e}")
            print("Random generation will be used:")
            generator = BuildingGenerator()
            building_3d, rooms_3d, doors_3d,people_type_mapping = generator.generate_building()
    else:
        # 随机生成 (原方式)
